    ...
    def __init__(self, config, exposure, footprints, noiseImage=None, exposureId=None, log=None):
        ...
        for maskname in ['THISDET', 'OTHERDET']:
            try:
                ...
                if self.log: self.log.trace('Mask plane "%s" already existed', maskname)
            except:
            ...
            if self.log: self.log.trace('Mask plane "%s": plane %i, bitmask %i = 0x%x',
        ...
        if self.log: self.log.trace('Using noise generator: %s', str(noisegen))
    ...
    def getNoiseGenerator(self, exposure, noiseImage, noiseMeanVar, exposureId=None):
        ...
        if noiseMeanVar is not None:
            try:
                ...
                if self.log: self.log.trace('Using passed-in noise mean = %g, variance = %g -> stdev %g',
                                            noiseMean, noiseVar, noiseStd)
                return FixedGaussianNoiseGenerator(noiseMean, noiseStd, rand=rand)
            except:
                if self.log: self.log.trace('Failed to cast passed-in noiseMeanVar to floats: %s',
                                            str(noiseMeanVar))
        ...
        if noiseSource == 'meta':
            ...
            try:
                ...
                if self.log: self.log.trace('Using noise variance = (BGMEAN = %g) from exposure metadata',
                                            bgMean)
                return FixedGaussianNoiseGenerator(offset, noiseStd, rand=rand)
            except:
                if self.log: self.log.trace('Failed to get BGMEAN from exposure metadata')

        if noiseSource == 'variance':
            if self.log: self.log.trace('Will draw noise according to the variance plane.')
        ...
        if self.log: self.log.trace("Measured from image: clipped mean = %g, stdev = %g",
                                    noiseMean,noiseStd)
